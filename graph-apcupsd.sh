...
# Check which fields are defined in rrd file
has_ITEMP=$(rrdinfo "$rrd_location" | grep -q '^ds\[ITEMP\]' && echo "yes")
has_OUTPUTV=$(rrdinfo "$rrd_location" | grep -q '^ds\[OUTPUTV\]' && echo "yes")
...
for period in $periods; do
    ...
    for canvas in 800/300 200/108/thumb; do
        ...
        #
        rrdtool_args=(
            graph "$rrd_graphdir/$period$suffix.png" \
